...
class _AddEditMotorPageState extends State<AddEditMotorPage> {
  ...
  void initState() {
    ...
    if (widget.motor != null) {
      ...
      _priceCtrl.text =
          (widget.motor!['harga'] as int?)?.toString() ??
          ''; // 'price' diganti 'harga'
      _ccCtrl.text = (widget.motor!['cc'] as int?)?.toString() ?? '';
      _yearCtrl.text =
          (widget.motor!['tahun_keluaran'] as int?)?.toString() ??
          ''; // Kolom baru
      _colorCtrl.text = widget.motor!['warna_motor'] ?? ''; // Kolom baru
      _descCtrl.text =
          widget.motor!['deskripsi'] ?? ''; // 'description' diganti 'deskripsi'

      _selectedCategoryId = widget.motor!['category_id'];
      _existingImageUrl =
          widget.motor!['foto_motor']; // 'image_url' diganti 'foto_motor'
    }
  }
  ...
  Future<void> _saveMotor() async {
    ...
    try {
      ...
      final data = {
        ...
        'tahun_keluaran':
            int.tryParse(_yearCtrl.text) ?? DateTime.now().year, // Kolom baru
      ...
      if (widget.motor == null) {
        ...
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Motor berhasil ditambahkan!")),
          );
      } else {
        await supabase
            .from('motors')
        ...
        if (mounted)
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Motor berhasil diperbarui!")),
          );
      }
                    ...
                    items: _categories
                        .map(
                          (cat) => DropdownMenuItem<String>(
                            ...
                            child: Text(
                              cat['nama_kategori'] as String? ?? 'N/A',
                            ),
                          ),
                        )
                        .toList(),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    ...
                    foregroundColor: Colors.white, // <<< WAJIB!
                  ...
                  child: _isLoading
                      ...
                      : Text(
                          isEditing ? "PERBARUI MOTOR" : "TAMBAH MOTOR",
